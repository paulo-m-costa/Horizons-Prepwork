...
towers.isValidBoard = function(board) {
  if (!towers.isArray(board)){return false;}
  if (board.length != 3){return false;}
  var sum = 0;
  var zeroTrack = 0;
  for (var i = 0; i < 3; i++){
    if (!towers.isArray(board[i])){return false;}
    if (board[i].length == 3 && board[i][0] <= board[i][1] || board[i][1] <= board[i][2]){return false;}
    if (board[i].length == 2 && board[i][0] <= board[i][1]){return false;}
    for (var j = 0; j < board[i].length; j++){
      if (board[i][j] > 2){return false;}
      if (board[i][j] == 0){zeroTrack++;}
      sum += board[i][j];
    }
  }
  if (sum != 3 || zeroTrack != 1){return false;}
  return true;
}
...
towers.makeMove = function(fromTower, toTower) {
  if (fromTower.length == 0){throw "Empty tower.";}
  var copy = towers.copyBoard(towers.game.board);
  copy[toTower].push(copy[fromTower].pop());
  if (!towers.isValidBoard(copy)){throw "Invalid move."}
  else {towers.game.board = copy;}
  towers.drawGame(towers.game.board);
  if(copy[2].length == 3){towers.win();}
}
