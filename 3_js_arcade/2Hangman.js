...
hangman.getRandomWord = function() {
  return hangman.words[hangman.getRandomInteger(0, hangman.words.length - 1)];
}
...
hangman.getGameStatus = function(word, guesses) {
  ...
  for (var g = 0; g < guesses.length; g++){
    if(hangman.isGuessWrong(word, guesses[g])){
      wrongCount++;
    }
  }
  if (wrongCount >= hangman.maxWrongGuesses){return 'lose';}
  var unique = word.split("").filter(function(x, n, s) { return s.indexOf(x) == n });
  if(guesses.length - wrongCount >= unique.length){return 'win';}
  return 'in_progress';
}
...
hangman.makeGuess = function(letter) {
  ...
  if(status == 'lose' || status == 'win'){throw 'Game over!';}
  else{
